...
const categories = ['الكل', ...Array.from(new Set(perfumes.map(p => p.category)))]
const brands = ['الكل', ...Array.from(new Set(perfumes.map(p => p.brand)))]
const genders = [
...
export default function ProductsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">جميع العطور</h1>
        <p className="text-gray-500">{filtered.length} عطر متاح</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        >
          ...
          {activeFiltersCount > 0 && (
            <span className="absolute -top-2 -left-2 bg-black text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">
              {activeFiltersCount}
            </span>
          )}
        </Button>
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          {selectedGender !== 'all' && (
            <Badge variant="secondary" className="gap-1 cursor-pointer" onClick={() => setSelectedGender('all')}>
              {genders.find(g => g.value === selectedGender)?.label}
              <X className="w-3 h-3" />
            </Badge>
          )}
          {selectedBrand !== 'الكل' && (
            <Badge variant="secondary" className="gap-1 cursor-pointer" onClick={() => setSelectedBrand('الكل')}>
              {selectedBrand} <X className="w-3 h-3" />
            </Badge>
          )}
          {selectedCategory !== 'الكل' && (
            <Badge variant="secondary" className="gap-1 cursor-pointer" onClick={() => setSelectedCategory('الكل')}>
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map(p => <ProductCard key={p.id} perfume={p} />)}
        </div>
      ) : (
